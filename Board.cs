...
namespace Chess
{
    class Board
    {
        ...
        public static bool isAccessAble(int y, int x) => y < 8 && y >= 0 && x < 10 && x >= 0;
        private void SetPieces(int y, ChessColor color)
        {
            tiles[y, 0].piece = new ChessPiece(PieceKind.Rook, color);
            tiles[y, 1].piece = new ChessPiece(PieceKind.Angel, color);
            tiles[y, 2].piece = new ChessPiece(PieceKind.Knight, color);
            ...
            tiles[y, 6].piece = new ChessPiece(PieceKind.Bishop, color);
            tiles[y, 7].piece = new ChessPiece(PieceKind.Knight, color);
            tiles[y, 8].piece = new ChessPiece(PieceKind.Angel, color);
            tiles[y, 9].piece = new ChessPiece(PieceKind.Rook, color);
            y += y == 0 ? 1 : -1;
            for (int x = 0; x < 10; x++)
